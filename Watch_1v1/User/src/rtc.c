...
u8 RTC_Init(void)
{
	...
	if(RTC_Read_BKR(0)!=0X5050)		//是否第一次配置?
	{
		...
		RTC->ISR&=~(1<<7);			//退出RTC初始化模式
		RTC->WPR=0xFF;				//使能RTC寄存器写保护  
		RTC_Set_Time(14,58,56,0);	//设置时间
		RTC_Set_Date(17,9,5,1);	//设置日期
		RTC_Set_AlarmA(4,15,24,10);	//设置闹钟时间
...
{ 
	...
	My_NVIC_Init(2,2,2,RTC_Alarm_IRQn); //抢占2，子优先级2，组2 
}
//周期性唤醒定时器设置
...
{ 
	...
	My_NVIC_Init(2,2,2,RTC_WKUP_IRQn); //抢占2，子优先级2，组2 
}
